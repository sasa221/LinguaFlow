...
async function startServer(shouldListen = true) {
  const app = express();
  const PORT = 3000;
  ...
  wss.on('connection', async (clientWs: WebSocket) => {
    ...
    clientWs.on('message', async (data: any) => {
      try {
        ...
        if (msg.type === 'init') {
          const { language, nativeLanguage, level, scenario, voice, coachingLevel } = msg;

          const systemPrompt = `You are playing the role of "${scenario?.partnerRole || 'Native Conversational Partner'}" in the setting: "${scenario?.setting || 'Everyday life'}".
The user is a language learner at CEFR level "${level || 'A2'}" learning "${language || 'Spanish'}".
Their native language is "${nativeLanguage || 'Arabic (Egyptian)'}".

Live Coaching Mode: "${coachingLevel || 'balanced'}".

Rules for spoken live voice interaction:
1. Speak exclusively in ${language} naturally as the character.
2. Keep each spoken turn short, natural, and conversational (1-2 sentences for A1-A2, 2-3 sentences for B1-B2).
3. Do NOT lecture or give long monologues. Prompt the learner to speak.
4. If the learner struggles or asks for help in ${nativeLanguage}, give a brief, friendly 1-sentence hint in ${nativeLanguage}, then immediately resume the roleplay in ${language}.
5. Start immediately in-character with an engaging greeting appropriate for the setting.`;
          ...
          try {
            session = await ai.live.connect({
              ...
              callbacks: {
                ...
                onclose: () => {
                  if (clientWs.readyState === WebSocket.OPEN) {
                    clientWs.send(JSON.stringify({ type: 'ended' }));
                  }
                },
              },
            });
            ...
            clientWs.send(
              JSON.stringify({
                type: 'error',
                message: connErr.message || 'Could not connect to Gemini Live voice stream',
              })
            );
          }
        }

        // 2. Real-time User Audio streaming (16kHz PCM Little-Endian)
        else if (msg.type === 'audio' && msg.audio && session && isSessionReady) {
          try {
        ...
        // 3. Fallback text input during live call
        else if (msg.type === 'text' && msg.text && session && isSessionReady) {
          try {
            session.sendRealtimeInput({
              text: msg.text,
            });
          } catch (textErr) {
            console.error('Error sending text to Live API:', textErr);
          }
        }
      } catch (err) {
...
// Vercel Express/WebSocket deployments import the HTTP server instead of
// opening a port. Local development still listens on port 3000.
const shouldListen = process.env.VERCEL !== '1';
const server = await startServer(shouldListen);
